            ...
            return Promise.props(requests)
                .then(function (response) {
                    ...
                    locales.forEach(function (locale) {
                        ...
                        fs.writeFile(outfile, response[locale]);
                    });
                    if (that.options.showStats) {
                        console.log('Downloaded ' + locales.length + ' locales: ', locales.join(', '));
                    }
                    return true;
                })
                .catch(function (error) {
...
API.prototype.makeRequest = function (settings) {
    ...
    request(requestOptions, function (error, rawResponse, body) {
        ...
        if (rawResponse.statusCode === 404) {
            return resolver.reject('Path was not found: ', settings.path);
        }
